...
def generate_scoped_combined_xml(df_01, df_15, scope, depr_key="ZSLM", fiscal_year="2026",
    ...
    for _, row01 in df_01.iterrows():
        ...
        for area in ["01", "15"]:
            ...
            capcol = cap_date_01 if area == "01" else cap_date_15
            amount_posted = ""
            if src_row is not None and capcol is not None and cap_date_on_or_after(src_row, capcol, fy_start_cutoff):
                amount_posted = str(src_row.iloc[acq_col]).strip() if acq_col is not None else ""
...
if uploaded_file:
    try:
        ...
        with colE:
            ...
            st.caption("Asset Transaction Type = 100, Sequence No. = 1, Reference Date = 1st of the fiscal year — all fixed per spec. Amount Posted = Cumulated Acquisition Value, only for assets capitalized on/after that date.")
            ...
            with open(out_fn, "wb") as f:
                ...
                for _, row01 in df_01.iterrows():
                    ...
                    for area in ["01", "15"]:
                        ...
                        capcol = cap_date_01 if area == "01" else cap_date_15
                        amount_posted = ""
                        if src_row is not None and capcol is not None and cap_date_on_or_after(src_row, capcol, fy_start_cutoff2):
                            amount_posted = str(src_row.iloc[acq_col]).strip() if acq_col is not None else ""

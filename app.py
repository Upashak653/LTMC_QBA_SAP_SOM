...
"""
Pure utility functions used throughout the LTMC mapper app: date parsing/
formatting, dynamic column resolution, sign handling, header-row detection,
and the shared validation-report helpers. No Streamlit UI flow lives here —
only reusable, independently-testable logic and display of results.
"""
...
def format_2dp(val):
    """Format a numeric string to exactly 2 decimal places (e.g. Cumulative
    Values' Acquisition Value and Depreciation columns, per LTMC spec).
    Leaves blank/non-numeric values unchanged; preserves the sign of an
    already-negative value (works fine composed after negate_num)."""
    s = str(val).strip()
    if not s or s.lower() == "nan":
        return s
    try:
...
def cap_date_on_or_after(row, cap_col, cutoff_str):
    """True if the given row's capitalization date is on/after cutoff_str
    (a 'YYYY-MM-DD' string), used for the Transactions tab's Amount Posted
    rule ("capitalisation date from 01.01.2026")."""
    if cap_col is None:
        return False
    d = parse_date_safe(row.iloc[cap_col])
    cutoff = parse_date_safe(cutoff_str)
    return pd.notna(d) and pd.notna(cutoff) and d >= cutoff
...
"""
Target-schema constants (matching the real SAP LTMC template exactly) and
the self-contained 9-tab combined-XML generator used by the Main Asset /
Sub Asset pages. Also retains some earlier, now-unused scaffolding
(write_letter_mapped_worksheet and its field-letter constants) exactly as
it was in the original single-file app — dead code, kept as-is rather than
removed, to avoid changing behavior during this file split.
"""
...
def generate_scoped_combined_xml(df_01, df_15, scope, depr_key="ZSLM", fiscal_year="2026",
                                  currency="INR", depr_posted_until="6", asset_txn_type="",
                                  seq_no="", ref_date=""):
    """
    Builds the full 9-tab combined LTMC XML, scoped to either "main" assets
    (Asset Subnumber == "0") or "sub" assets (Asset Subnumber >= 1), per the
    person's Main Asset / Sub Asset page split. Subnumber is resolved by
    header text ("Asset Subnumber"), same as everywhere else in the app —
    verified reliable across both tested client files, both of which happen
    to have it at column E, consistent with the person's description.

    Returns (xml_bytes, stats_dict) where stats_dict has per-tab row counts
    and the count of assets included/excluded by the scope filter.
    """
    idx_01_asset = find_col_dynamic(df_01, ["External/Legacy Asset Number", "Asset Number"])
    idx_01_comp = find_col_dynamic(df_01, ["Company Code"])
    idx_01_sub = find_col_dynamic(df_01, ["Asset Subnumber"])
    idx_01_acq = find_col_dynamic(df_01, ["Cumulated Acquisition Value"])
    idx_01_dep = find_col_dynamic(df_01, ["Accumulated Ordinary Depreciation"])
    idx_15_asset = find_col_dynamic(df_15, ["External/Legacy Asset Number", "Asset Number"])
    idx_15_acq = find_col_dynamic(df_15, ["Cumulated Acquisition Value"])
    idx_15_dep = find_col_dynamic(df_15, ["Accumulated Ordinary Depreciation"])
    ...
    # Main Asset Legacy Number (source Column D in both tabs) — for sub-assets,
    # every tab's "External/Legacy Asset Number" should show the PARENT/main
    # asset's legacy number instead of the sub-asset's own number, per spec.
    # Falls back to the asset's own number when Column D is blank (verified:
    # 100% blank in both tested client files today), so this stays safe and
    # produces identical output to before until a client's file actually
    # populates this field.
    main_asset_legacy_01 = find_col_dynamic(df_01, ["Main Asset Legacy Number"])
    if main_asset_legacy_01 is None:
        main_asset_legacy_01 = 3 if df_01.shape[1] > 3 else None  # fallback: literal Column D
    main_asset_legacy_15 = find_col_dynamic(df_15, ["Main Asset Legacy Number"])
    if main_asset_legacy_15 is None:
        main_asset_legacy_15 = 3 if df_15.shape[1] > 3 else None  # fallback: literal Column D

    def display_asset_number(own_asset_key, source_row, legacy_col):
        """For sub-asset scope only: return the Main Asset Legacy Number if
        the source has one populated, else fall back to the asset's own
        number (own_asset_key) so nothing goes blank."""
        if scope != "sub" or source_row is None or legacy_col is None:
            return own_asset_key
        v = str(source_row.iloc[legacy_col]).strip()
        if not v or v.lower() == "nan":
            return own_asset_key
        return v.split(".")[0]

    c_comp = find_col_dynamic(df_15, ["Company Code"])
    c_asset = idx_15_asset
    c_sub = idx_15_sub = find_col_dynamic(df_15, ["Asset Subnumber"])
    ...
    cost_center_col = find_col_dynamic(df_15, ["Cost Center Code", "Cost Center"])
    if cost_center_col is None:
        cost_center_col = 9 if df_15.shape[1] > 9 else None  # fallback: literal Column J
    plant_col = find_col_dynamic(df_15, ["Plant"])
    if plant_col is None:
        plant_col = 21 if df_15.shape[1] > 21 else None  # fallback: literal Column V
    ...
    def sub_matches_scope(sub_val):
        ...
        try:
            ...
            n = 0
        return n == 0 if scope == "main" else n >= 1

    area15_lookup = {}
    for _, r in df_15.iterrows():
        k = str(r.iloc[idx_15_asset]).strip().split(".")[0]
        if k and k != "nan":
            area15_lookup[k] = r
    ...
    included_assets, excluded_assets = 0, 0

    def write_sheet(f, name, headers, rows):
    ...
    buf.write(b'<?mso-application progid="Excel.Sheet"?>\n')
    buf.write(b'<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"\n')
    buf.write(b' xmlns:o="urn:schemas-microsoft-com:office:office"\n')
    buf.write(b' xmlns:x="urn:schemas-microsoft-com:office:excel"\n')
    buf.write(b' xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet">\n')
    ...
    for _, row01 in df_01.iterrows():
        ...
        if not sub_matches_scope(sub_num):
            continue
        comp_code = str(row01.iloc[idx_01_comp]).strip()
        val_01_acq = format_2dp(row01.iloc[idx_01_acq])
        val_01_dep = format_2dp(negate_num(row01.iloc[idx_01_dep]))
        matched15 = area15_lookup.get(asset_key)
        val_15_acq = format_2dp(matched15.iloc[idx_15_acq]) if matched15 is not None else "0.00"
        val_15_dep = format_2dp(negate_num(matched15.iloc[idx_15_dep])) if matched15 is not None else "0.00"
        for area, acq_val, dep_val in [("01", val_01_acq, val_01_dep), ("15", val_15_acq, val_15_dep)]:
            ...
            if cap_source_row is not None and cap_date_on_or_after(cap_source_row, cap_source_col, "2026-01-01"):
                continue
            legacy_col = main_asset_legacy_01 if area == "01" else main_asset_legacy_15
            disp_asset = display_asset_number(asset_key, cap_source_row, legacy_col)
            cv_rows.append([
                comp_code, disp_asset, sub_num, area, fiscal_year, acq_val, "", "",
                dep_val, "", "", "", "", "", "", currency,
            ])
    write_sheet(buf, "Cumulative Values", TARGET_COLUMNS, cv_rows)
    stats["Cumulative Values"] = len(cv_rows)
    ...
    md_rows = []
    n_included_assets, n_excluded_assets = 0, 0
    for _, row in df_15.iterrows():
        asset_key = get_val(row, c_asset).split(".")[0]
        if not asset_key:
            continue
        sub_num = get_val(row, c_sub) or "0"
        if not sub_matches_scope(sub_num):
            n_excluded_assets += 1
            continue
        if cap_date_15 is not None and cap_date_on_or_after(row, cap_date_15, "2026-01-01"):
            continue  # 2026+ acquisitions belong in Transactions only, not this tab
        n_included_assets += 1
        disp_asset = display_asset_number(asset_key, row, main_asset_legacy_15)
        md_rows.append([
            get_val(row, c_comp), disp_asset, sub_or_zero(row), get_val(row, c_class),
    ...
    write_sheet(buf, "Master Details", md_headers, md_rows)
    stats["Master Details"] = len(md_rows)
    included_assets, excluded_assets = n_included_assets, n_excluded_assets
    ...
    for _, row in df_15.iterrows():
        ...
        disp_asset = display_asset_number(asset_key, row, main_asset_legacy_15)
        inv_rows.append([
            get_val(row, c_comp), disp_asset, sub_or_zero(row),
            get_val(row, c_inv_lastdate), get_val(row, c_inv_suppl), get_val(row, c_inv_ind),
        ])
    write_sheet(buf, "Inventory", inv_headers, inv_rows)
    stats["Inventory"] = len(inv_rows)
    ...
    for _, row in df_15.iterrows():
        ...
        is_sub_asset = False
        try:
            is_sub_asset = int(float(sub_num)) >= 1
        except ValueError:
            pass
        if scope == "sub":
            # In the Sub Asset file specifically, Origin's Manufacturer of
            # Asset (target Column F) is populated from the source's own
            # Column B (External/Legacy Asset Number) — per explicit
            # instruction. Note this puts the asset's own ID number in this
            # field, not a real manufacturer name.
            mfr_val = asset_key
        else:
            mfr_val = get_val(row, c_mfr) if is_sub_asset else ""
        disp_asset = display_asset_number(asset_key, row, main_asset_legacy_15)
        origin_rows.append([
            get_val(row, c_comp), disp_asset, sub_or_zero(row), get_val(row, c_vendor_no),
            get_val(row, c_vendor), mfr_val, get_val(row, c_purchnew), get_val(row, c_tradeid),
    ...
    write_sheet(buf, "Origin", origin_headers, origin_rows)
    stats["Origin"] = len(origin_rows)
    ...
    for _, row01 in df_01.iterrows():
        ...
        # Exclude entirely if the asset's earliest capitalization (across both
        # tabs) is 2026 or later — that asset is a 2026+ acquisition and
        # belongs in Transactions only.
        if dates and min(dates) >= parse_date_safe("2026-01-01"):
            continue
        # Prefer I GAAP's Main Asset Legacy Number (matches this app's
        # subnumber-driven hierarchy), falling back to IFRS's own.
        disp_asset = display_asset_number(asset_key, matched15, main_asset_legacy_15)
        if disp_asset == asset_key:
            disp_asset = display_asset_number(asset_key, row01, main_asset_legacy_01)
        pi_rows.append([comp, disp_asset, sub_num, cap_final, "", "", "", ""])
    write_sheet(buf, "Posting Information", pi_headers, pi_rows)
    stats["Posting Information"] = len(pi_rows)
    ...
    for _, row in df_15.iterrows():
        ...
        plant_val = get_val(row, plant_col)
        disp_asset = display_asset_number(asset_key, row, main_asset_legacy_15)
        td_rows.append([
            get_val(row, c_comp), disp_asset, sub_or_zero(row), "", cc_val,
            "", "", "", "", plant_val,
        ] + [""] * (len(td_headers) - 10))
    write_sheet(buf, "Time-Dependent Data", td_headers, td_rows)
    stats["Time-Dependent Data"] = len(td_rows)
    ...
    for _, row01 in df_01.iterrows():
        ...
        for area in ["01", "15"]:
            ...
            if capcol is not None and cap_date_on_or_after(src_row, capcol, "2026-01-01"):
                continue  # 2026+ acquisitions belong in Transactions only, not this tab
            ...
            legacy_col = main_asset_legacy_01 if area == "01" else main_asset_legacy_15
            disp_asset = display_asset_number(asset_key, src_row, legacy_col)
            da_rows.append([
                comp, disp_asset, sub_num, area, "", depr_key,
    ...
    write_sheet(buf, "Depreciation Areas", da_headers, da_rows)
    stats["Depreciation Areas"] = len(da_rows)
    ...
    for _, row01 in df_01.iterrows():
        ...
        for area in ["01", "15"]:
            ...
            disp_asset = display_asset_number(asset_key, src_row, legacy_col)
            pv_rows.append([
                comp, disp_asset, sub_num, area, fiscal_year, "",
                negate_num(src_row.iloc[pod]) if pod is not None else "",
                "", "", "", "", "", currency, depr_posted_until,
            ])
    write_sheet(buf, "Posted Values", pv_headers, pv_rows)
    stats["Posted Values"] = len(pv_rows)
    ...
    ref_date_computed = f"01-01-{fiscal_year}"
    fy_start_cutoff = f"{fiscal_year}-01-01"
    tx_rows = []
    for _, row01 in df_01.iterrows():
        asset_key = str(row01.iloc[idx_01_asset]).strip().split(".")[0]
        if not asset_key or asset_key == "nan":
        ...
        for area in ["01", "15"]:
            ...
            if area == "15" and matched15 is None:
                continue
            acq_col = idx_01_acq if area == "01" else idx_15_acq
            capcol = cap_date_01 if area == "01" else cap_date_15
            # Only assets capitalized on/after the fiscal-year start appear in
            # Transactions at all — this is a row filter, not just a blank fallback.
            if src_row is None or capcol is None or not cap_date_on_or_after(src_row, capcol, fy_start_cutoff):
                continue
            amount_posted = format_2dp(src_row.iloc[acq_col]) if acq_col is not None else ""
            legacy_col = main_asset_legacy_01 if area == "01" else main_asset_legacy_15
            disp_asset = display_asset_number(asset_key, src_row, legacy_col)
            tx_rows.append([comp, disp_asset, sub_num, area, "100", fiscal_year, "1", ref_date_computed, amount_posted, currency])
    write_sheet(buf, "Transactions", tx_headers, tx_rows)
    stats["Transactions"] = len(tx_rows)

    buf.write(b'</Workbook>\n')

    stats["assets_included"] = included_assets
    stats["assets_excluded"] = excluded_assets
...
"""
The Main Asset / Sub Asset pages: a simplified upload -> auto-detect -> 
generate flow, reusing the same dynamic column resolution and combined-XML
generation logic as the Full Pipeline, but scoped by Asset Subnumber.
"""

def render_scoped_asset_page(scope):
    """Renders the Main Asset or Sub Asset page: upload -> auto-detect tabs ->
    generate the scoped combined 9-tab XML -> download."""
    label = "Main Asset" if scope == "main" else "Sub Asset"
    condition_text = "Asset Subnumber = 0" if scope == "main" else "Asset Subnumber \u2265 1"
    st.title(f"🚀 {label} — SAP LTMC Export")
    st.write(f"Same 9-tab LTMC pipeline as the full mapper, filtered to only assets where **{condition_text}** (source Column E — resolved by header text, matching every tested client file).")

    st.subheader("📁 Upload Source Workbook")
    up = st.file_uploader("Upload Source Excel File (.xlsx)", type=["xlsx"], key=f"uploader_{scope}")
    if not up:
        return

    xl_file = pd.ExcelFile(up)
    available_sheets = xl_file.sheet_names
    st.success(f"📖 Detected {len(available_sheets)} tabs.")

    c1, c2 = st.columns(2)
    with c1:
        default_ifrs = next((i for i, s in enumerate(available_sheets) if "IFRS" in s.upper()), 0)
        area01_tab = st.selectbox("Tab for Depreciation Area 01 (IFRS):", options=available_sheets, index=default_ifrs, key=f"tab01_{scope}")
    with c2:
        default_igaap = next((i for i, s in enumerate(available_sheets) if "GAAP" in s.upper()), min(1, len(available_sheets) - 1))
        area15_tab = st.selectbox("Tab for Depreciation Area 15 (IGAAP):", options=available_sheets, index=default_igaap, key=f"tab15_{scope}")

    skip01 = detect_header_row(up, area01_tab)
    skip15 = detect_header_row(up, area15_tab)
    sk1, sk2 = st.columns(2)
    with sk1:
        skip_rows_01 = st.number_input(f"Header rows to skip — {area01_tab} (auto: {skip01}):", min_value=0, value=skip01, key=f"skip01_{scope}")
    with sk2:
        skip_rows_15 = st.number_input(f"Header rows to skip — {area15_tab} (auto: {skip15}):", min_value=0, value=skip15, key=f"skip15_{scope}")

    up.seek(0)
    df_01 = pd.read_excel(up, sheet_name=area01_tab, skiprows=skip_rows_01, dtype=str).dropna(how="all").fillna("")
    up.seek(0)
    df_15 = pd.read_excel(up, sheet_name=area15_tab, skiprows=skip_rows_15, dtype=str).dropna(how="all").fillna("")
    ...
    with p1:
        depr_key = st.text_input("Depreciation Key", value="ZSLM", key=f"depr_key_{scope}")
    with p2:
        fy = st.text_input("Current Fiscal Year", value="2026", key=f"fy_{scope}")
    with p3:
        currency = st.text_input("Currency Key", value="INR", key=f"cur_{scope}")
    with p4:
        depr_until = st.text_input("Depr. Posted Until", value="6", key=f"dpu_{scope}")

    if st.button(f"Generate {label} Combined XML", type="primary", key=f"gen_{scope}"):
        xml_bytes, stats = generate_scoped_combined_xml(
            df_01, df_15, scope, depr_key=depr_key, fiscal_year=fy, currency=currency, depr_posted_until=depr_until,
        )
        st.success(f"✅ {label} export generated — {stats['assets_included']} assets included, {stats['assets_excluded']} excluded (wrong scope).")
        for tab_name in ["Cumulative Values", "Master Details", "Inventory", "Origin", "Posting Information",
                         "Time-Dependent Data", "Depreciation Areas", "Posted Values", "Transactions"]:
            st.caption(f"{tab_name}: {stats[tab_name]:,} rows")
        st.download_button(
            f"📥 Download {label} Combined LTMC XML", data=xml_bytes,
            file_name=f"LTMC_{label.replace(' ', '_')}_Combined.xml", mime="text/xml", key=f"dl_{scope}",
        )
...
# --- PAGE NAVIGATION: Full Pipeline / Main Asset / Sub Asset ---
# Placed after all module-level helper/generator functions above are defined,
# so the two new scoped pages can safely call them.
st.sidebar.markdown("### Navigation")
page_choice = st.sidebar.radio(
    "Choose a page",
    ["Full Pipeline (9 tabs, manual mapping)", "Main Asset", "Sub Asset"],
    key="page_choice",
)

if page_choice == "Main Asset":
    render_scoped_asset_page("main")
    st.stop()
elif page_choice == "Sub Asset":
    render_scoped_asset_page("sub")
    st.stop()
# else: fall through to the existing full pipeline below, unchanged.
    ...
    try:
            ...
            if len(keys_01) > 0 and len(overlap) == 0:
                ...
                with open(output_filename, "wb") as f:
                    ...
                    for idx, row_01 in df_01.iterrows():
                        ...
                        val_01_acq = format_2dp(row_01.iloc[idx_01_acq])
                        val_01_dep = format_2dp(negate_num(row_01.iloc[idx_01_dep]))

                        matched_row_15 = area15_lookup.get(asset_key, None)
                        if matched_row_15 is not None:
                            val_15_acq = format_2dp(matched_row_15.iloc[idx_15_acq])
                            val_15_dep = format_2dp(negate_num(matched_row_15.iloc[idx_15_dep]))
                        else:
                            val_15_acq = "0.00"
                            val_15_dep = "0.00"
                ...
                if total_records_written > 0:
                    ...
                    src_acq_total = sum(
                        ...
                        if not cap_date_on_or_after(row_01, cap_date_01, "2026-01-01")
                    )
                    src_dep_total = -sum(
                    ...
                    src_acq_total_15 = sum(
                        ...
                        if not cap_date_on_or_after(row_15, cap_date_15, "2026-01-01")
                    )
        ...
        if anchor_comp is None or anchor_asset is None:
            ...
            def write_positional_tab_xml(worksheet_name, field_specs, out_filename):
                """
                field_specs: list of (header_label, resolved_col_index_or_fixed_value_or_None_or_callable)
                Each entry writes one column, in order, at its fixed target
                position — resolved_col_index_or_fixed_value_or_None_or_callable can be:
                  - an int      -> pull from that source column index
                  - a str       -> literal fixed value (e.g. "X", "0")
                  - a callable  -> called with the source row, returns the value
                                    (used for row-conditional fields, e.g.
                                    "only for sub-assets")
                  - None        -> leave blank
                """
                def clean_xml(val):
            ...
            with btn3:
                ...
                if st.button("Generate Origin XML"):
                    ...
                    def sub_is_nonzero(row):
                        s = str(row.iloc[anchor_sub]).strip() if anchor_sub is not None else "0"
                        try:
                            return int(float(s or "0")) >= 1
                        except ValueError:
                            return False
                    ...
                    field_specs = [
                        ...
                        ("Manufacturer of Asset", (lambda row: get_val(row, org_mfr) if sub_is_nonzero(row) else "")),
                    ...
                    with st.expander("Origin — resolved source columns"):
                        for label, spec in field_specs:
                            if callable(spec):
                                mfr_status = f"{df_15.columns[org_mfr].split(chr(10))[0][:50]}" if org_mfr is not None else "not found in source"
                                st.write(f"- {label} → {mfr_status} (only populated for sub-assets, per spec)")
                            elif isinstance(spec, int):
                                st.write(f"- {label} → {df_15.columns[spec].split(chr(10))[0][:50]}")
                            elif isinstance(spec, str):
                                st.write(f"- {label} → fixed value '{spec}'")
        ...
        # Cost Center & Plant: resolved by header text first (robust across
        # clients — verified Column J/V mean different things in different
        # client files: J = "Cost Center Code" for one client, "Equipment ID"
        # for another), falling back to the literal source column (J for Cost
        # Center, V for Plant) only if no header match is found, per the
        # explicit column references given.
        cost_center_col = find_col_dynamic(df_15, ["Cost Center Code", "Cost Center"])
        if cost_center_col is None:
            cost_center_col = 9 if df_15.shape[1] > 9 else None  # fallback: literal Column J
        plant_col = find_col_dynamic(df_15, ["Plant"])
            ...
            if st.button("Generate Time-Dependent Data XML"):
                ...
                with open(out_fn, "wb") as f:
                    ...
                    for _, srow in df_15.iterrows():
                        ...
                        if cc_val == "nan":
                            cc_val = ""
                        plant_val = str(srow.iloc[plant_col]).strip() if plant_col is not None else ""
                        if plant_val == "nan":
                            plant_val = ""
                        row_vals = [
                            comp, asset_key, sub, "", cc_val,
                            "", "", "", "", plant_val,
                        ] + [""] * (len(td_headers) - 10)
                        f.write(b'   <Row>\n')
                        for v in row_vals:
                    ...
                    f.write(b' </Worksheet>\n')
                    f.write(b'</Workbook>\n')
                st.success(f"✅ {count:,} rows written (Cost Center + Plant populated; all other optional fields left blank, per spec).")
                report = validate_rows(
        ...
        with colE:
            ...
            st.caption("Only assets capitalized on/after the fiscal year start appear here at all. Asset Transaction Type = 100, Sequence No. = 1, Reference Date = 1st of the fiscal year, Amount Posted = Cumulated Acquisition Value.")
            if st.button("Generate Transactions XML"):
                ...
                ref_date_computed = f"01-01-{fy_step6_input}"
                fy_start_cutoff = f"{fy_step6_input}-01-01"
                ...
                with open(out_fn, "wb") as f:
                    ...
                    for _, row01 in df_01.iterrows():
                        ...
                        for area in ["01", "15"]:
                            ...
                            acq_col = idx_01_acq if area == "01" else idx_15_acq
                            capcol = cap_date_01 if area == "01" else cap_date_15
                            # Only assets capitalized on/after the fiscal-year
                            # start appear in Transactions at all — this is a
                            # row filter, not just a blank Amount Posted fallback.
                            if src_row is None or capcol is None or not cap_date_on_or_after(src_row, capcol, fy_start_cutoff):
                                continue
                            amount_posted = format_2dp(src_row.iloc[acq_col]) if acq_col is not None else ""
                            row_vals = [
                                comp, asset_key, sub, area, "100",
                                fy_step6_input, "1", ref_date_computed, amount_posted, "",
                            ]
                            f.write(b'   <Row>\n')
                            for v in row_vals:
                                f.write(f'    <Cell><Data ss:Type="String">{clean_xml(v)}</Data></Cell>\n'.encode("utf-8"))
        ...
        if st.button("📥 Generate Combined LTMC XML (All Tabs)", type="primary"):
            ...
            with open(out_fn, "wb") as f:
                ...
                for _, row in df_15.iterrows():
                    ...
                    if not asset_key:
                        continue
                    if cap_date_15 is not None and cap_date_on_or_after(row, cap_date_15, "2026-01-01"):
                        continue  # 2026+ acquisitions belong in Transactions only
                    md_rows.append([
                ...
                for _, row in df_15.iterrows():
                    ...
                    if cap_date_15 is not None and cap_date_on_or_after(row, cap_date_15, "2026-01-01"):
                        continue  # 2026+ acquisitions belong in Transactions only
                    sub_val = sub_or_zero(row)
                    is_sub_asset = False
                    try:
                        is_sub_asset = int(float(sub_val)) >= 1
                    except ValueError:
                        pass
                    mfr_val = get_val(row, c_mfr) if is_sub_asset else ""
                    origin_rows.append([
                        get_val(row, c_comp), asset_key, sub_val, get_val(row, c_vendor_no),
                        get_val(row, c_vendor), mfr_val, get_val(row, c_purchnew), get_val(row, c_tradeid),
                        get_val(row, c_country), get_val(row, c_typename), get_val(row, c_acqdate),
                        get_val(row, c_acqyr), get_val(row, c_acqval), get_val(row, c_inhouse),
                    ])
                ...
                for _, row in df_15.iterrows():
                    ...
                    plant_val = get_val(row, plant_col)
                    td_rows.append([
                        get_val(row, c_comp), asset_key, sub_or_zero(row), "", cc_val,
                        "", "", "", "", plant_val,
                    ] + [""] * (len(td_headers2) - 10))
                ...
                ref_date_computed2 = f"01-01-{fy_step6_input}"
                fy_start_cutoff2 = f"{fy_step6_input}-01-01"
                tx_rows = []
                for _, row01 in df_01.iterrows():
                    asset_key = str(row01.iloc[idx_01_asset]).strip().split(".")[0]
                    if not asset_key or asset_key == "nan":
                    ...
                    for area in ["01", "15"]:
                        ...
                        acq_col = idx_01_acq if area == "01" else idx_15_acq
                        capcol = cap_date_01 if area == "01" else cap_date_15
                        if src_row is None or capcol is None or not cap_date_on_or_after(src_row, capcol, fy_start_cutoff2):
                            continue
                        amount_posted = format_2dp(src_row.iloc[acq_col]) if acq_col is not None else ""
                        tx_rows.append([comp, asset_key, sub, area, "100", fy_step6_input, "1", ref_date_computed2, amount_posted, ""])
            ...
            with st.expander("📋 Final Validation Report — click to expand", expanded=True):
                ...
                src_acq_total = sum(
                    ...
                    if not cap_date_on_or_after(r, cap_date_01, "2026-01-01")
                )
                src_dep_total = -sum(
                ...
                # Manufacturer is only populated for sub-assets (Subnumber >= 1) per
                # spec, so the expected map must blank it for main assets too —
                # otherwise this check would falsely flag every main asset as a
                # mismatch (source has a value, generated correctly leaves it blank).
                mfr_map = {}
                for _, r in df_15.iterrows():
                    k = str(r.iloc[idx_15_asset]).strip().split(".")[0]
                    if not k or k == "nan":
                        continue
                    sub_v = get_val(r, c_sub) or "0"
                    try:
                        is_sub = int(float(sub_v)) >= 1
                    except ValueError:
                        is_sub = False
                    mfr_map[k] = get_val(r, c_mfr) if is_sub else ""
                ...
                origin_checks = [
                    ...
                    ("Origin — Manufacturer of Asset (sub-assets only)", mfr_map, origin_rows, asset_i_org, 5, None, None),

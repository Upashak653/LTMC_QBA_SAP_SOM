        ...
        if st.button("Generate Production LTMC Upload XML", type="primary"):
            ...
            if len(keys_01) > 0 and len(overlap) == 0:
                ...
                if total_records_written > 0:
                    ...
                    src_acq_total = sum(to_num(row_01.iloc[idx_01_acq]) for _, row_01 in df_01.iterrows())
                    src_dep_total = sum(to_num(row_01.iloc[idx_01_dep]) for _, row_01 in df_01.iterrows())
                    src_acq_total_15 = sum(to_num(row_15.iloc[idx_15_acq]) for _, row_15 in df_15.iterrows())
                    src_dep_total_15 = sum(to_num(row_15.iloc[idx_15_dep]) for _, row_15 in df_15.iterrows())
            ...
            if st.button("Generate Posting Information XML"):
                ...
                with open(out_fn, "wb") as f:
                    ...
                    for _, row01 in df_01.iterrows():
                        ...
                        cap_date_final = min(dates).strftime("%Y-%m-%d") if dates else ""
                ...
                with open(out_fn, "wb") as f:
                    ...
                    for _, srow in df_15.iterrows():
                        ...
                        if not sub or sub == "nan":
                            sub = "0"
                        row_vals = [comp, asset_key, sub] + [""] * (len(td_headers) - 3)
                        f.write(b'   <Row>\n')
                        for v in row_vals:
                            f.write(f'    <Cell><Data ss:Type="String">{clean_xml(v)}</Data></Cell>\n'.encode("utf-8"))
                        f.write(b'   </Row>\n')
                    ...
                    for _, row01 in df_01.iterrows():
                        ...
                        for area in ["01", "15"]:
                            ...
                            row_vals = [
                                ...
                                "", "", "", "",
                                str(src_row.iloc[capcol]).strip() if capcol is not None else "",
                            ]
                            f.write(b'   <Row>\n')
                            for v in row_vals:
                                f.write(f'    <Cell><Data ss:Type="String">{clean_xml(v)}</Data></Cell>\n'.encode("utf-8"))
                    ...
                    for _, row01 in df_01.iterrows():
                        ...
                        for area in ["01", "15"]:
                            ...
                            row_vals = [
                                ...
                                str(src_row.iloc[pod]).strip() if pod is not None else "",
                                "", "", "", "", "",
        ...
        if st.button("📥 Generate Combined LTMC XML (All Tabs)", type="primary"):
            ...
            with open(out_fn, "wb") as f:
                ...
                for _, row01 in df_01.iterrows():
                    ...
                    val_01_dep = str(row01.iloc[idx_01_dep]).strip()
                    matched15 = area15_lookup_s6.get(asset_key)
                    val_15_acq = str(matched15.iloc[idx_15_acq]).strip() if matched15 is not None else "0.00"
                    val_15_dep = str(matched15.iloc[idx_15_dep]).strip() if matched15 is not None else "0.00"
                    for area, acq_val, dep_val in [("01", val_01_acq, val_01_dep), ("15", val_15_acq, val_15_dep)]:
                        cv_rows.append([
                ...
                # --- 3. Inventory ---
                inv_headers2 = ["Company Code", "External/Legacy Asset Number", "Asset Subnumber"]
                inv_rows = []
                for _, row in df_15.iterrows():
                    asset_key = get_val(row, c_asset).split(".")[0]
                    if not asset_key:
                        continue
                    inv_rows.append([get_val(row, c_comp), asset_key, sub_or_zero(row)])
                ...
                for _, row01 in df_01.iterrows():
                    ...
                    d15 = parse_date_safe(matched15.iloc[cap_date_15]) if (matched15 is not None and cap_date_15 is not None) else pd.NaT
                    dates = [d for d in [d01, d15] if pd.notna(d)]
                    cap_final = min(dates).strftime("%Y-%m-%d") if dates else ""
                ...
                for _, row in df_15.iterrows():
                    ...
                    if not asset_key:
                        continue
                    td_rows.append([get_val(row, c_comp), asset_key, sub_or_zero(row)] + [""] * (len(td_headers2) - 3))
            ...
            with st.expander("📋 Final Validation Report — click to expand", expanded=True):
                ...
                src_acq_total = sum(to_num(r.iloc[idx_01_acq]) for _, r in df_01.iterrows())
                src_dep_total = sum(to_num(r.iloc[idx_01_dep]) for _, r in df_01.iterrows())
                ...
                src_dep_map_01 = build_asset_value_map(df_01, idx_01_asset, idx_01_dep)
                src_pod_map_01 = build_asset_value_map(df_01, idx_01_asset, posted_ord_dep_01)
                ...
                for r in pi_rows:
                    ...
                    dates = [d for d in [d01, d15] if pd.notna(d)]
                    expected = min(dates).strftime("%Y-%m-%d") if dates else ""
